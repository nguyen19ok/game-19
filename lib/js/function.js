...
//   Load_Game(1,'game-taixiu','ngocrong');
     ...
     Load_Game(6,'khung_minigame','minigame');
...
function resetpass() {
    $['ajax']({
        url: '/guest/xuli.html?resetpass',
        type: 'POST',
        data: $('#form_rspass')['find']('select, textarea, input')['serialize'](),
        success: function (log) {
          
                $("#log_rspass").html(log.text);
    ...
    $.ajax(
        {
            ...
            success : function(d)
            {
                ...
                $("#list_chat_"+d.to).append('<li class="odd msg_sent"><div class="chat-content"><div class="box bg-light-info">'+d.text+'</div><br></div><div class="chat-time" ducnghia="'+d.thoigian+'">Vừa song</div></li>');
...
{
    if($(".chat-windows #user-chat"+id).length >=1) //nếu đã có khung chát này...
    {
    ...
    $.ajax(
        {
            ...
            success : function(d)
            {
                ...
                s += "<div class='chat-body'><ul class='b-c-content' id='list_chat_"+t+"'>"+list+"</ul></div>"; /*Center*/
                
                s += "<div class='chat-footer'><input type='text' id='input_"+t+"' placeholder='nhập nội dung' class='form-control' onkeydown = 'if (event.keyCode == 13) send_sms("+t+")'></div>"; /*Foot*/
    ...
    $.ajax(
        {
            ...
            success : function(d)
            {
                ...
                var html = '<li><div class="drop-title text-white bg-danger"><h4 class="m-b-0 m-t-5">'+d.total+' tin nhắn mới</h4><span class="font-light">Tin nhắn</span></div> </li>';
...
setInterval(function(){getmoney()},1000);