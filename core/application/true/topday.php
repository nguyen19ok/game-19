...
<h2>Top thắng ngày</h2>
</div>'; 


$dataTop = $mysqli->query("SELECT * FROM `nguoichoi` ORDER by win DESC LIMIT $start, 31");
$log = array();
$currtip=0;
while($top=$dataTop->fetch_assoc())
{
    ...
    {
   $msg.='<tr id="count"><td>TOP'.$currtip.' - '.$top['taikhoan'].' </td> <td>'.number_format($top['win']).' </td>    </tr>';
    }
    $currtip++;
}
